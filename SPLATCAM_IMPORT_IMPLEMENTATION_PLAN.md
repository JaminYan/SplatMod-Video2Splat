...
“仅导入检查”期间界面显示不定进度条和已用时间，明确提示当前正在读取 RGB、计算清晰度并检查相机轨迹；由于检查在单个 Rust 任务中完成，当前不伪造百分比进度。

Rust 侧现在保存一次性的预检报告缓存：源目录的相对文件名、大小和修改时间组成指纹，缓存有效期为 5 分钟；路径变化、指纹变化或过期都会丢弃缓存。点击“导入并训练”时只有指纹仍匹配且未过期才复用报告，否则重新调用 `inspect_export`。标准化文本模型也复用同一报告，避免在训练入口内再次解析点云和解码 RGB。缓存仅存在当前进程内，不改变来源文件。
