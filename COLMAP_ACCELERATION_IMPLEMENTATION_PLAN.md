...
2026-09-05 增量实现：补充 SfM 报告达到至少 80% 注册率，且每份已验证候补素材至少有一张图像注册后，才允许“采用补充 SfM 并继续训练”。训练输入先从隔离 attempt 准备，成功生成并校验 PLY 后才提升帧目录、COLMAP 数据库与稀疏模型；失败或取消保留原 `needsSupplement` 状态和 attempt 证据。重开应用会从报告恢复验证结果，继续按钮不会重复启动 SfM。JPG/JPEG/PNG 均纳入输入图像分母，避免 PNG 补拍导致注册率失真。
