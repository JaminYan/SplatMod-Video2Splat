...
fn projection_statistics(
    ...
    for pose in poses {
        ...
        for point in &sample {
            let z = r[2][0] * point[0] + r[2][1] * point[1] + r[2][2] * point[2] + pose.t[2];
            if z > 0.0 {
                positive += 1;
            }
        }
    ...
    for point in &sample {
        let visible = poses.iter().any(|pose| {
            ...
            let r = rotation_matrix(pose.q);
            let x = r[0][0] * point[0] + r[0][1] * point[1] + r[0][2] * point[2] + pose.t[0];
            let y = r[1][0] * point[0] + r[1][1] * point[1] + r[1][2] * point[2] + pose.t[1];
            let z = r[2][0] * point[0] + r[2][1] * point[1] + r[2][2] * point[2] + pose.t[2];
            if z <= 0.0 {
                return false;
            }
