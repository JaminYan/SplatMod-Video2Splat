...
$requiredFiles = @(
    ...
    "scripts/setup-engines.ps1",
    "scripts/verify-engines.ps1",
    "scripts/download-colmap-cuda.ps1"
)
...
Assert-True ($package.scripts.PSObject.Properties.Name -contains "download:colmap-cuda") "package.json must expose the download:colmap-cuda script."
...
Assert-True ($manifest.schemaVersion -ge 3) "Engine manifest schemaVersion must include optional COLMAP entries."
Assert-True ($manifest.engines.Count -eq 5) "Engine manifest must list FFmpeg, 3× COLMAP, and Brush."
$expectedEngines = @{
    "FFmpeg / FFprobe" = @{
...
Assert-Contains $thirdParty "FFmpeg / FFprobe" "THIRD_PARTY_NOTICES.txt"
Assert-Contains $thirdParty "COLMAP (CPU/no-CUDA)" "THIRD_PARTY_NOTICES.txt"
Assert-Contains $thirdParty "COLMAP (CUDA)" "THIRD_PARTY_NOTICES.txt"
Assert-Contains $thirdParty "COLMAP (CASPAR CUDA)" "THIRD_PARTY_NOTICES.txt"
Assert-Contains $thirdParty "Brush" "THIRD_PARTY_NOTICES.txt"
...
Write-Host "Verified OOOSplat license metadata and 5 direct engine notices."
