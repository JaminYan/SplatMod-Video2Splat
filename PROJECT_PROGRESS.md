...
- 0.49：补充 SfM attempt 通过质量门禁后可续接标准 Brush/gsplat 训练与导出，原始 attempt 和诊断证据保持不变。
- 0.49：新增历史策略评分、policy sweep 与共享 warmup 自动选择脚本，输出 JSON 供复核，不自动改变默认策略。
- 0.49：WD-R 10k 暂列推荐实验；多视图新增点门禁的跨素材结果不稳定，默认继续关闭。
